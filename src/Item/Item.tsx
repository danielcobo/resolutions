...
const Remove = function Remove(props: { id: string }) {
  const remove = useStore((state: Store) => state.remove);
  ...
  return (
    <button onClick={del} data-id={props.id}>
      X
    </button>
  );
};
...
const Item = function Item(props: Item) {
  const select = useStore((state: Store) => state.select);
